...
"""Site Transfer Form had no Company/Letter Head fields at all, unlike most
other IR process forms tied to an Employee - every existing record (and
every signed print of one) used the site's default Letter Head instead of
the Employee's own Company's. The doctype now carries and auto-populates
both fields going forward (see SiteTransferForm.validate()); this backfills
every existing record from its own Employee, once.
"""
...
def execute():
    ...
    # an IN list) - fetch every row with an Employee set and check
    # blankness in Python instead.
    rows = frappe.get_all(
        DOCTYPE,
