...
function render_previous_matrix(frm) {
  ...
  // Align display order with current table rows where possible (for side-by-side mental comparison)
  const order_index = {};
  ...
  prev_rows.forEach((r) => {
    ...
    // prefer stored total if present, else computed
...
frappe.ui.form.on("Employment Equity Target", {
  ...
  refresh: function (frm) {
    ...
    // Re-render so both HTML views stay in sync with grid changes
    render_matrix(frm);
    render_previous_matrix(frm);
  },
