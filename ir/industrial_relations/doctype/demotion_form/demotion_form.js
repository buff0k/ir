...
frappe.ui.form.on('Demotion Form', {
    ...
    linked_disciplinary_action: function(frm) {
        if (frm.doc.linked_disciplinary_action) {
            frappe.call({
                method: 'ir.industrial_relations.doctype.demotion_form.demotion_form.fetch_disciplinary_action_data',
                args: {
    ...
    linked_poor_performance: function(frm) {
        if (frm.doc.linked_poor_performance && !frm.doc.linked_poor_performance_processed) {
            frappe.call({
                method: 'ir.industrial_relations.doctype.demotion_form.demotion_form.fetch_poor_performance_data',
                args: {
    ...
    before_submit: function(frm) {
        ...
        if (!frm.doc.signed_demotion) {
            frappe.msgprint(__('You must attach the signed demotion before submitting.'));
