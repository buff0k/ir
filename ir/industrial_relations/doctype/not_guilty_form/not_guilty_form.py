...
class NotGuiltyForm(Document):
    def autoname(self):
        ...
        if len(existing_docs) == 0:
            ...
            for doc in existing_docs:
                ...
                if doc.name.startswith(f"NO-{linked_field}-"):
            ...
            self.name = f"NO-{linked_field}-{new_revision}"
    ...
    def set_outcome_in_linked_documents(self):
        ...
        if linked_doc_name and linked_doctype:
            ...
            if linked_doc.docstatus == 0:  # Non-submitted document
                linked_doc.outcome = self.type_of_not_guilty
                linked_doc.outcome_date = self.outcome_date
                linked_doc.save(ignore_permissions=True)
            else:  # Submitted document
                linked_doc.db_set("outcome", self.type_of_not_guilty)
                linked_doc.db_set("outcome_date", self.outcome_date)

                # Create a manual version entry for the submitted document
                create_manual_version(linked_doc, "outcome", previous_outcome, self.type_of_not_guilty)
            ...
            frappe.msgprint(
                ...
                .format(linked_doc_name, linked_doctype, self.type_of_not_guilty, self.outcome_date),
                alert=True
            )
...
@frappe.whitelist()
def make_not_incap(source_name, target_doc=None):
    from frappe.model.mapper import get_mapped_doc
