...
frappe.ui.form.on('Pay Deduction Form', {
    ...
    linked_poor_performance: function(frm) {
        if (frm.doc.linked_poor_performance && !frm.doc.linked_poor_performance_processed) {
            frappe.call({
                method: 'ir.industrial_relations.doctype.pay_deduction_form.pay_deduction_form.fetch_poor_performance_data',
                args: {
            ...
            frappe.call({
                ...
                callback: function(r) {
                    if (r.message) {
                        ...
                        let msg = `The linked document ${linked_doc_name} (${linked_doctype}) currently has an outcome: ${outcome_str} and outcome date: ${outcome_date_str}. These will be overwritten with pay deduction type: ${frm.doc.pay_deduction_type} and outcome date: ${frm.doc.outcome_date}. Do you want to proceed?`;
