...
frappe.ui.form.on("Pay Deduction Form", {
    ...
    linked_intervention(frm) {
        ...
        frappe.call({
            method:
                "ir.industrial_relations.doctype.pay_deduction_form.pay_deduction_form.fetch_intervention_data",
            args: {
    ...
    company(frm) {
        ...
        frappe.call({
            method:
                "ir.industrial_relations.doctype.pay_deduction_form.pay_deduction_form.fetch_company_letter_head",
    ...
    before_submit(frm) {
        if (!frm.doc.attached_signed_pay_deduction) {
            frappe.throw(
                __("You must attach the signed pay deduction before submitting.")
            );
        }
...
function confirm_outcome_change(frm, mode) {
    ...
    return new Promise((resolve, reject) => {
        frappe.call({
            method:
                "ir.industrial_relations.doctype.pay_deduction_form.pay_deduction_form.get_linked_outcome",
            args: {
