...
class PayDeductionForm(Document):
    def autoname(self):
        ...
        prefix = f"PDED-{self.linked_intervention}"
    ...
    def validate(self):
        ...
        if self.deduction_amount is None:
            frappe.throw(_("Monetary Value of Deduction is required."))

        if self.deduction_amount < 0:
            frappe.throw(_("Monetary Value of Deduction cannot be negative."))
    ...
    def before_submit(self):
        ...
        if not self.attached_signed_pay_deduction:
            frappe.throw(_("You must attach the signed pay deduction before submitting."))
    ...
    def _set_linked_outcome(self):
        ...
        if linked_doc.meta.has_field("outcome"):
            values["outcome"] = self.pay_deduction_type
...
@frappe.whitelist()
def create_pay_deduction_form(source_name, source_doctype):
    _validate_source(source_name, source_doctype)

    target = frappe.new_doc("Pay Deduction Form")
    target.ir_intervention = source_doctype
    target.linked_intervention = source_name
    target.applied_rights = "Pay Deduction"
...
def _build_outcome_start(doc):
    text = _("The employee's pay deduction applies from {0}").format(
        frappe.format_value(doc.from_date, {"fieldtype": "Date"})
    )
    if doc.deduction_amount is not None:
        text += _(" by {0}").format(
            frappe.format_value(doc.deduction_amount, {"fieldtype": "Currency"})
        )
    return f"{text}."
...
def _build_outcome_end(doc):
    if not doc.to_date:
        return None
    return _("The employee's pay deduction applies until {0}.").format(
