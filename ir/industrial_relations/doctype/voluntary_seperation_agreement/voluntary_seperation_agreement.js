...
frappe.ui.form.on("Voluntary Seperation Agreement", {
    ...
    auth_manager: function(frm) {
        if (frm.doc.auth_manager) {
            frappe.call({
                method: 'ir.industrial_relations.doctype.voluntary_seperation_agreement.voluntary_seperation_agreement.fetch_authorizor_names',
                args: {
                    auth_manager: frm.doc.auth_manager,
                },
                callback: function(r) {
                    if (r.message) {
