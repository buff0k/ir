...
def on_create(doc, method=None):
    _notify_ir(doc=doc, action="created")
...
def on_update(doc, method=None):
    ...
    if changed:
        _notify_ir(doc=doc, action="updated", changed_fields=changed)

def on_submit(doc, method=None):
    _notify_ir(doc=doc, action="completed")
...
def _collect_recipients(doc):
    ...
    Includes all users with IR roles + the document creator (owner).
...
def _notify_ir(doc, action, changed_fields=None):
